...
// Builds one Date per batch from the schedule's available slots, using
// an organic "slow start -> build up -> peak mid-window -> taper off"
// density instead of an even spread. Slots near the middle of the
// window get weighted to receive proportionally more batches; slots
// near the start and end get fewer - so cumulative delivery looks like
// real engagement building up and cresting, not a flat, mechanical line.
//
// Works the same way whether batchCount is smaller or larger than the
// number of real slots: each slot's weighted share of batchCount is
// computed first (a bell curve centered on the window's midpoint), then
// that many timestamps are placed at/around that slot. When a slot ends
// up needing more than one batch, extra occurrences get a jitter window
// that widens with how many are stacked there, so a busy mid-window
// slot doesn't get several batches at the exact identical minute.
function buildOrganicScheduleTimes(allSlots, count) {
  const n = allSlots.length;
  if (count <= 0) return [];
  if (n === 1) return Array.from({ length: count }, () => new Date(allSlots[0].getTime()));

  const center = (n - 1) / 2;
  const sigma = n / 4; // controls how tight the mid-window peak is
  const weights = allSlots.map((_, i) => Math.exp(-((i - center) ** 2) / (2 * sigma * sigma)));
  const totalWeight = weights.reduce((a, b) => a + b, 0);

  // Largest-remainder rounding so the per-slot counts sum to exactly
  // `count`, even though the raw weighted shares are fractional.
  ...
  const times = [];
  for (let i = 0; i < n; i++) {
    const repCount = counts[i];
    const maxJitter = repCount > 1 ? Math.min(30, Math.ceil(repCount / 2)) : 0;
    for (let rep = 0; rep < repCount; rep++) {
      const jitter = rep === 0 ? 0 : randomInt(-maxJitter, maxJitter);
      times.push(new Date(allSlots[i].getTime() + jitter * 60000));
    }
  }
