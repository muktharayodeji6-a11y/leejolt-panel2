...
async function loadServices() {
  try {
    ...
    const res = await fetch(`${ORDER_WORKER_URL}?type=tk-services`, {
      method: "GET",
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("completeByValue")?.addEventListener("input", updateEstimatedCost);
  document.getElementById("completeByUnit")?.addEventListener("change", updateEstimatedCost);
});
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    const completeByValue = parseFloat(document.getElementById("completeByValue").value);
    const completeByUnit = document.getElementById("completeByUnit").value;
    const completeByHours = completeByUnit === "days" ? completeByValue * 24 : completeByValue;
    const rows = getFilledRows();
    ...
    if (!completeByValue || completeByHours < 0.25) {
      errorEl.textContent = "Enter how long from now this plan should complete (minimum 15 minutes).";
      errorEl.classList.add("visible");
      return;
    }
    if (rows.length === 0) {
