...
// from each service's own provider catalog every time so rates/min/max
...
const FAVORITE_SERVICES = [
  { id: "3907", label: "Views", minOverride: 50 },
  { id: "3772", label: "Likes" },
  { id: "3446", label: "Saves" },
...
function renderServiceRows() {
  ...
  FAVORITE_SERVICES.forEach((fav, i) => {
    const provider = fav.provider || "betalogs";
    const service = servicesCatalog.find(
      (s) => String(s.service) === fav.id && s.provider === provider
    );
    if (!service) {
      html += `
        <div class="field" data-row="${i}" style="border-top: 1px solid var(--panel-border); padding-top: 14px; margin-top: 4px;">
          ...
          <p style="color: var(--red); font-size: 13px; margin: 4px 0;">Service ID ${fav.id} not found in the ${provider} catalog - check it still exists.</p>
        </div>`;
      return;
    }

    const batchMin = fav.minOverride ? Math.max(service.min, fav.minOverride) : service.min;
...
async function loadServices() {
  try {
    ...
    servicesCatalog = data.services.map((s) => ({ ...s, provider: "betalogs" }));
    renderServiceRows();
  } catch (err) {
...
// ChickletBoost and TheKclaut catalogs require an auth token (matches
// the Worker's cb-services/tk-services endpoints), so these run once
// authReady fires rather than on page load like the Betalogs catalog.
async function loadProviderCatalog(type, providerName, idToken) {
  try {
    const res = await fetch(`${ORDER_WORKER_URL}?type=${type}`, {
      headers: { Authorization: `Bearer ${idToken}` }
    });
    const data = await res.json();
    if (!res.ok || !Array.isArray(data.services)) throw new Error(data.error || `Failed to load ${providerName} services`);

    const tagged = data.services.map((s) => ({ ...s, provider: providerName }));
    servicesCatalog = servicesCatalog.concat(tagged);
    renderServiceRows(); // re-render now that this provider's rows can resolve
  } catch (err) {
    console.error(`Failed to load ${providerName} services:`, err);
  }
}
...
function getFilledRows() {
  ...
  document.querySelectorAll("#serviceRows [data-row]").forEach((rowEl) => {
    ...
    rows.push({
      ...
      provider: rowEl.dataset.provider || "betalogs",
      rate: parseFloat(rowEl.dataset.rate || 0),
      min: parseInt(rowEl.dataset.min || 0, 10),
      batchMin: parseInt(rowEl.dataset.batchMin || rowEl.dataset.min || 0, 10),
...
// TikTok Views/Likes live on ChickletBoost/TheKclaut, whose catalog
// endpoints require an ID token - fetch those once the user is signed in.
document.addEventListener("authReady", async (e) => {
  try {
    const idToken = await e.detail.user.getIdToken();
    loadProviderCatalog("cb-services", "chickletboost", idToken);
    loadProviderCatalog("tk-services", "thekclaut", idToken);
  } catch (err) {
    console.error("Failed to load provider catalogs:", err);
  }
});
      ...
      <div style="max-height: 220px; overflow-y: auto;">
        ${batches
          .map((b, i) => {
            ...
            const failureNote = b.status === "failed" && b.failureReason
              ? `<div style="color: var(--red); font-size: 11px; padding: 2px 0 4px; opacity: 0.85;">${b.failureReason}</div>`
              : "";
            return `
          <div style="padding: 6px 0; border-top: 1px solid var(--panel-border); font-size: 12px; ${strike}">
