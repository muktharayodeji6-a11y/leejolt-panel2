...
function wireOverview() {
  ...
  if (processBtn) {
    processBtn.addEventListener("click", async () => {
      ...
      processResultEl.textContent = "";
      try {
        const idToken = await auth.currentUser.getIdToken();
        const res = await fetch(`${ORDER_WORKER_URL}?type=process`, {
          headers: { Authorization: `Bearer ${idToken}` }
        });
        const data = await res.json();
        processResultEl.textContent = JSON.stringify(data, null, 2);
      } catch (err) {
        processResultEl.textContent = "Error: " + err.message;
      } finally {
        processBtn.disabled = false;
        processBtn.textContent = "Process batches now";
      }
    });
  }

  const balanceEl = document.getElementById("betalogsBalance");
  if (balanceEl) {
    auth.currentUser
      ?.getIdToken()
      .then((idToken) =>
        fetch(`${ORDER_WORKER_URL}?type=balance`, {
          headers: { Authorization: `Bearer ${idToken}` }
        })
      )
      .then(async (res) => {
        ...
        balanceEl.textContent = data.balance
          ...
          : "Unavailable: " + JSON.stringify(data);
      })
      .catch((err) => {
        console.error("Failed to load Betalogs balance:", err);
        balanceEl.textContent = "Error: " + err.message;
      });
  }

  const tkBalanceEl = document.getElementById("thekclautBalance");
  if (tkBalanceEl) {
    auth.currentUser
      ?.getIdToken()
      .then((idToken) =>
        fetch(`${ORDER_WORKER_URL}?type=tk-balance`, {
          headers: { Authorization: `Bearer ${idToken}` }
        })
      )
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) throw new Error(`HTTP ${res.status}: ${JSON.stringify(data)}`);
        tkBalanceEl.textContent = data.balance
          ? "\u20a6" + Number(data.balance).toLocaleString("en-NG", { minimumFractionDigits: 2 })
          : "Unavailable: " + JSON.stringify(data);
      })
      .catch((err) => {
        console.error("Failed to load TheKclaut balance:", err);
        tkBalanceEl.textContent = "Error: " + err.message;
      });
  }
