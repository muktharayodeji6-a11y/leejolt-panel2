...
function generateBatchPlan(totalQuantity, serviceMin, windowMs) {
  ...
  // Always leave headroom below the theoretical max batch count - hitting
  // it exactly forces every batch to precisely the minimum with zero room
  // to vary in size, which is its own kind of giveaway.
  ...
  // up so gaps typically stay under ~45-60 minutes, still within the same
  // headroom cap.
  if (windowMs) {
...
function assignScheduleTimes(count, windowMs, startOffsetMs = 0) {
  ...
  const times = [];
  const MIN_GAP_MS = randomInt(15, 30) * 60 * 1000; // 15-30 min gaps

  for (let i = 0; i < count; i++) {
    let scheduledTime;
    if (i === 0) {
      // First batch: fires immediately within 0.5-2 seconds
      scheduledTime = now + randomInt(500, 2000);
    } else {
      // Subsequent batches: space them 15-30 min apart
      scheduledTime = times[i - 1].getTime() + MIN_GAP_MS;
    }

    let candidate = new Date(scheduledTime);
    candidate = avoidQuietHours(candidate);

    // Try to hit peak hours, but don't shift too far from natural tight schedule
    if (!PEAK_HOURS_WAT.includes(watHourOf(candidate)) && Math.random() < 0.2) {
      const targetHour = PEAK_HOURS_WAT[randomInt(0, PEAK_HOURS_WAT.length - 1)];
      const hourDiff = targetHour - watHourOf(candidate);
      // Only shift if less than 3 hours away (don't stretch delivery)
      if (Math.abs(hourDiff) <= 3 || (hourDiff > 12)) {
        candidate.setUTCHours(candidate.getUTCHours() + hourDiff);
        candidate = avoidQuietHours(candidate);
      }
    }
...
async function verifyFirebaseIdToken(idToken, projectId) {
  ...
  if (!payload.aud || !payload.aud.includes(projectId)) {
    throw new Error("Token aud claim does not match project");
  }
...
async function getFirestoreAccessToken(env) {
  const now = Date.now();
  if (cachedAccessToken && cachedAccessTokenExp > now + 60000) {
...
function pemToArrayBuffer(pem) {
  const lines = pem.replace(/-----(BEGIN|END) [A-Z ]+ KEY-----/g, "").trim().split(/\s+/);
