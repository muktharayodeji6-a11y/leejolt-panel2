...
// Server-side safety net: enforces the real per-service minimums even
// if the frontend ever fails to send (or sends a wrong) `serviceMin`.
// Confirmed against orders.js FAVORITE_SERVICES as of July 2026:
//   - Instagram Views (Betalogs, id 3532): min 50
//   - TikTok Views (ChickletBoost, id 5233): min 100
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (dualProviderIds) {
        let betalogsMin = Math.max(1, serviceMin || BATCH_MIN);
        const betalogsOverrideMin = SERVICE_MIN_OVERRIDES[`betalogs:${serviceIdBetalogs}`];
        if (betalogsOverrideMin) {
          betalogsMin = Math.max(betalogsMin, betalogsOverrideMin);
        }
        ...
        let effectiveMin = Math.max(1, serviceMin || BATCH_MIN);
        const overrideMin = SERVICE_MIN_OVERRIDES[`${targetProvider}:${serviceId}`];
        if (overrideMin) {
          effectiveMin = Math.max(effectiveMin, overrideMin);
        }
...
// Batches for short (<24h) windows are spaced by a random interval
// (55-75 min) picked fresh between each batch, rather than one-per-
// clock-hour. This makes batch COUNT purely a function of how much
// time fits in the window - not quantity - so a big order and a
// small order in the same window get the same number of batches,
// just bigger or smaller ones, matching real posting cadence.
...
// rather than just the first `count` - so capping batch count for
// variance headroom doesn't bunch batches into the start of the window.
function pickEvenSubset(times, count) {
...
function planShortWindowBatches(quantity, effectiveMin, windowMs, offsetMs) {
  ...
  // Walk forward from the start, spacing each batch 55-75 min after
  // the last, until the window runs out. Always places at least one.
  const allTimes = [];
  ...
  const maxBatchesForVariance = Math.max(1, Math.floor(quantity / (effectiveMin * VARIANCE_HEADROOM_FACTOR)));
...
// Maps an order's quantity onto batches spaced by the 55-75 min
// interval walk (planShortWindowBatches), regardless of how long the
// window is. Batch COUNT is purely a function of how many ~65-min
// gaps fit in the window; batch SIZE scales with quantity/batchCount.
// This replaces the old fixed-schedule-bank + narrow-band system for
// every window length - short orders and long multi-day orders alike
// now go through the same interval-based planner.
//
// scheduleIndex is still accepted (and still stored on the order doc
// by the caller) for backward compatibility with existing refill code
// paths, but it's no longer used to pick batch timing here.
...
// Dual-provider version of the interval planner: batch COUNT still
// comes purely from the 55-75 min interval walk (same as the single-
// provider planner), but each batch independently rolls a provider
// (respecting canUseThekclaut) and is sized above *that* provider's
// own minimum, so a Betalogs batch looks like 50-70 and a TheKclaut
// batch looks like 100-140, never mixed up.
//
