...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ---- Create a new order ----
      const { orderId, serviceId, link, quantity, completeBy, startOffsetMinutes, serviceMin, provider } = body;
...
function generateBatchPlan(totalQuantity, serviceMin, windowMs) {
  ...
  // Leave a little headroom below the theoretical max batch count so
  // sizes still vary, but don't be so conservative that a long window
  // can't get enough batches to actually spread across every day of it.
  const safeMaxBatches = Math.max(
    1,
    maxPossibleBatches - Math.max(1, Math.floor(maxPossibleBatches * 0.1))
  );

  let batchCount = Math.max(
    1,
    Math.min(safeMaxBatches, Math.ceil(totalQuantity / effectiveMax))
  );

  // If this service only ends up with a handful of batches spread across
  // a long window, gaps between them can stretch to several hours - or
  // even skip whole days entirely. Bump the batch count up so gaps stay
  // reasonable AND so every day of a multi-day window gets real coverage.
  if (windowMs) {
    const DAY_MS = 24 * 3600 * 1000;
    const daysInWindow = Math.max(1, Math.ceil(windowMs / DAY_MS));
    const MAX_GAP_MS = randomInt(45, 60) * 60 * 1000;
    const batchesForGap = Math.ceil(windowMs / MAX_GAP_MS) + 1;
    const batchesForDailySpread = daysInWindow * 2; // aim for ~2+/day minimum
    const batchesNeededForGoodCoverage = Math.max(batchesForGap, batchesForDailySpread);
...
function assignScheduleTimes(count, windowMs, startOffsetMs = 0) {
  ...
  const DAY_MS = 24 * 3600 * 1000;
  const totalDays = Math.max(1, Math.ceil(windowMs / DAY_MS));

  // Split batches across every calendar day the window touches, so a
  // multi-day order visibly delivers on day 1, day 2, day 3... instead
  // of a curve that (even unintentionally) skews early and can finish
  // well before the window is up. Each day gets a randomized share
  // around an even split - not a rigid count/totalDays - so it still
  // looks organic rather than metronomic.
  const dayWeights = Array.from({ length: totalDays }, () => 0.7 + Math.random() * 0.6);
  const weightSum = dayWeights.reduce((a, b) => a + b, 0);

  const dayCounts = dayWeights.map((w) => Math.floor((w / weightSum) * count));
  let assigned = dayCounts.reduce((a, b) => a + b, 0);
  // Hand out any remainder one at a time to random days until it adds up.
  let guard = 0;
  while (assigned < count && guard < 10000) {
    dayCounts[randomInt(0, totalDays - 1)]++;
    assigned++;
    guard++;
  }
  // (If count < totalDays, some days legitimately get zero - the order
  // is just too small to touch every single day, which is fine.)

  const times = [];
  for (let d = 0; d < totalDays; d++) {
    const dayStart = now + d * DAY_MS;
    const dayEnd = Math.min(now + windowMs, dayStart + DAY_MS);
    const dayWindowMs = Math.max(1, dayEnd - dayStart);

    for (let j = 0; j < dayCounts[d]; j++) {
      // Fully random position within this day's slice - not a fixed
      // sub-slot - so same-day batches don't line up in a predictable grid.
      let candidate = new Date(dayStart + Math.random() * dayWindowMs);
