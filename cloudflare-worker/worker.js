...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const { batchSizes, scheduleTimes } = planScheduleBatches(
        scheduleIndex,
        quantity,
        effectiveMin,
        remainingWindowMs,
...
// across them, respecting the service's real minimum per batch.
function planScheduleBatches(scheduleIndex, quantity, effectiveMin, windowMs, offsetMs) {
  ...
  const base = Math.floor(quantity / batchCount);
  const remainder = quantity - base * batchCount;
  const batchSizes = scheduleTimes.map((_, i) => (i === batchCount - 1 ? base + remainder : base));
