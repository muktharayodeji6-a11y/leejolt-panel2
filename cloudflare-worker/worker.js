...
// Per-service overrides for the drip-feed batch floor. Views is forced
// to a batch minimum of 50 regardless of whatever Betalogs reports as
// its live order minimum (which only governs the smallest allowed
// order size, not how we want to chop it into batches).
const SERVICE_MIN_OVERRIDES = { "2479": 50 }; // Views
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const batchSizes = generateBatchPlan(quantity, serviceMin, serviceId);
      const scheduleTimes = assignScheduleTimes(batchSizes.length, remainingWindowMs, offsetMs);

      const batchDocsData = batchSizes.map((size, i) => ({
        orderId,
        serviceId,
        link,
        quantity: size,
      ...
      await firestoreBatchCreate(env, accessToken, `orders/${orderId}/batches`, batchDocsData);
...
function generateBatchPlan(totalQuantity, serviceMin, serviceId) {
  // Follow each service's real minimum rather than a fixed floor - a
  // "likes" service with min=1 can trickle in fine, small batches, while
  // a "views" service with min=50 naturally gets bigger chunks. BATCH_MAX
  // still caps how big any single batch can get either way. A hardcoded
  // override (SERVICE_MIN_OVERRIDES) takes priority over whatever
  // Betalogs reports live, since that live value governs the smallest
  // allowed order size, not how we want to chop it into batches.
  const overrideMin = SERVICE_MIN_OVERRIDES[String(serviceId)];
  const effectiveMin = Math.max(1, overrideMin || serviceMin || BATCH_MIN);
  // Scale the batch-size ceiling to the service too: a low-minimum
  // service (likes, min=1) gets small, fine-grained batches; a
  // higher-minimum service (views, min=50) keeps the usual larger chunks.
  // Give the ceiling real headroom above the minimum (rather than a
  // fixed TYPICAL_BATCH_SIZE that can collapse batches to one uniform
  // size once the minimum itself grows past it) so batch sizes actually
  // vary randomly instead of clustering on a single number.
  const TYPICAL_BATCH_SIZE = 12;
  const effectiveMax = Math.min(
    BATCH_MAX,
    Math.max(TYPICAL_BATCH_SIZE, Math.ceil(effectiveMin * 1.8))
  );

  // How many batches can we possibly make without any of them
  // falling below the service's real minimum?
  const maxPossibleBatches = Math.floor(totalQuantity / effectiveMin);

  if (maxPossibleBatches <= 1) {
    return [totalQuantity]; // too small to split without violating the minimum
  }

  // Aim for batches near effectiveMax in size, capped by what's
  // actually achievable given the minimum constraint.
  ...
  for (let i = 0; i < batchCount; i++) {
    ...
    // Never leave less than the remaining batches would need at minimum size.
...
function assignScheduleTimes(count, windowMs, startOffsetMs = 0) {
  ...
  // a flat, even spread is itself a machine-like signature.
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Blend: half surge (early attention spike), half even spread
    // (so longer windows still carry real, visible activity late on,
    // not just a thin trickle after the first day).
...
async function firestoreBatchUpdateStatus(env, accessToken, docs, newStatus) {
  ...
  const CHUNK_SIZE = 400; // Firestore commit endpoint allows up to 500 writes
  let updatedCount = 0;
...
// Generates a Firestore-safe document ID for manually-named batch docs.
function generateBatchId() {
  return crypto.randomUUID().replace(/-/g, "").slice(0, 20);
}

// Creates many batch documents in as few HTTP requests as possible,
// using Firestore's :commit endpoint (single request can carry hundreds
// of writes) instead of one firestoreCreate POST per batch. Large orders
// (1000+ units, 80+ batches) used to blow past Cloudflare's ~50
// subrequest-per-invocation limit partway through the creation loop,
// silently dropping every batch after the cutoff and leaving the order
// stuck on "processing"/"pending" - same class of bug the cancel-order
// path already fixed with firestoreBatchUpdateStatus.
async function firestoreBatchCreate(env, accessToken, collectionPath, docsData) {
  const CHUNK_SIZE = 400; // Firestore commit endpoint allows up to 500 writes

  for (let i = 0; i < docsData.length; i += CHUNK_SIZE) {
    const chunk = docsData.slice(i, i + CHUNK_SIZE);
    const writes = chunk.map((data) => {
      const id = generateBatchId();
      const name = `projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/${collectionPath}/${id}`;
      return { update: { name, fields: jsToFirestoreFields(data) } };
    });
