...
// Builds one Date per batch by cycling through the schedule's slots as
// many times as needed. The first pass uses each slot's exact time;
// every additional pass through the same slots gets a small jitter
// (+/- 6 min) so repeated batches at "the same" scheduled moment don't
// all carry an identical timestamp.
function buildRepeatingScheduleTimes(allSlots, count) {
  const times = [];
  for (let i = 0; i < count; i++) {
    const baseSlot = allSlots[i % allSlots.length];
    const cycle = Math.floor(i / allSlots.length);
    if (cycle === 0) {
      times.push(new Date(baseSlot.getTime()));
    } else {
      const jitterMinutes = randomInt(-6, 6);
      times.push(new Date(baseSlot.getTime() + jitterMinutes * 60000));
    }
  }
  times.sort((a, b) => a - b);
  return times;
