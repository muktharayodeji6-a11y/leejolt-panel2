...
function generateBatchPlan(totalQuantity, serviceMin, windowMs) {
  ...
  // still caps how big any single batch can get either way.
  ...
  // Target a modest typical batch size (not just "whatever the service
  // minimum allows up to 60") so services with a low minimum actually
  // get split into several pieces instead of just 1-2 large chunks.
  const TYPICAL_BATCH_SIZE = 12;
  // If the minimum itself is already bigger than our typical target size,
  // still give it a real randomization range (not a fixed single number) -
  // otherwise every batch comes out exactly identical, which is itself
  // a dead giveaway.
  ...
  // a long window, the gaps between them can stretch to several hours -
  // dead silence that doesn't look like ongoing organic activity. Bump
  // the batch count up (still respecting the minimum) so no gap should
  // typically exceed ~45-60 minutes.
  if (windowMs) {
    const MAX_GAP_MS = randomInt(45, 60) * 60 * 1000;
    const batchesNeededForGoodCoverage = Math.ceil(windowMs / MAX_GAP_MS) + 1;
    // Cap at 80% of the theoretical max, not the full max - hitting the
    // exact ceiling forces every batch to precisely the minimum with zero
    // room left for size variation, which is its own kind of giveaway.
...
function assignScheduleTimes(count, windowMs, startOffsetMs = 0) {
  ...
  // above can still bunch a few together too tightly by chance,
  // especially early in the surge. A believable drip shouldn't have two
  // batches landing 30 seconds apart.
  const MIN_GAP_MS = randomInt(4, 9) * 60 * 1000; // 4-9 min, varies per order
