...
// shared 24h anchor schedule (SCHEDULE_BANK), which is spaced ~1.5-2h
// apart and too slow-looking for this service. Short, frequent batches
// instead: 18-35 min between sends, each sized 50-60.
const IG_VIEWS_BETALOGS_SERVICE_ID = "3907";
const IG_VIEWS_BATCH_MIN = 50;
const IG_VIEWS_BATCH_MAX = 60;
const IG_VIEWS_GAP_MIN_MINUTES = 18;
const IG_VIEWS_GAP_MAX_MINUTES = 35;
// Occasional outlier gaps so the spacing doesn't look like it's always
// drawn from the same tight band - real delivery has bursts (a couple
// batches minutes apart) and lulls (an hour+ gap) mixed in.
const IG_VIEWS_BURST_GAP_MIN_MINUTES = 3;
const IG_VIEWS_BURST_GAP_MAX_MINUTES = 15;
const IG_VIEWS_BURST_CHANCE = 0.2;
const IG_VIEWS_LULL_GAP_MIN_MINUTES = 40;
const IG_VIEWS_LULL_GAP_MAX_MINUTES = 90;
const IG_VIEWS_LULL_CHANCE = 0.15;
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      ...
      if (url.searchParams.get("type") === "progress") {
        try {
          ...
          const orderFields = firestoreValueToJS({ mapValue: { fields: orderRaw.fields } });

          const batchesUrl = `https://firestore.googleapis.com/v1/projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/orders/${orderId}/batches?pageSize=300`;
          const batchesRes = await fetch(batchesUrl, { headers: { Authorization: `Bearer ${accessToken}` } });
          const batchesRaw = await batchesRes.json();

          const batches = (batchesRaw.documents || []).map((doc) => {
            const f = firestoreValueToJS({ mapValue: { fields: doc.fields } });
            return {
              time: f.scheduledAt,
              quantity: f.quantity,
              status: f.status, // "pending" | "sent" | "failed"
            };
          });

          batches.sort((a, b) => new Date(a.time) - new Date(b.time));

          let cumulative = 0;
          const points = batches.map((b) => {
            if (b.status === "sent") cumulative += b.quantity;
            return {
              time: b.time,
              status: b.status,
          ...
          return corsResponse(
            ...
            jsonResponse({
              ...
              targetQuantity: orderFields.quantity || null,
              deliveredQuantity: orderFields.deliveredQuantity || 0,
              totalBatches: orderFields.totalBatches || batches.length,
              deliveredBatches: orderFields.deliveredBatches || 0,
              points,
            })
          );
        } catch (err) {
          console.error(err);
          return corsResponse(env, jsonResponse({ error: "Progress fetch failed: " + (err.message || String(err)) }, 500));
        }
      }
    ...
    try {
      ...
      const VALID_PROVIDERS = ["betalogs", "thekclaut", "chickletboost"];
      const targetProvider = VALID_PROVIDERS.includes(provider) ? provider : "betalogs"; // defaults to betalogs
      ...
      // Betalogs Instagram Views (single-provider path only, standalone
      // or as part of a growth-plan bundle) uses its own fast drip
      // pattern - see planIGViewsFastBatches - instead of the shared
      // SCHEDULE_BANK anchors every other service rides on.
      const isIGViewsFast = !dualProviderIds
        && targetProvider === "betalogs"
        && String(serviceId) === IG_VIEWS_BETALOGS_SERVICE_ID;
      ...
      } else if (isIGViewsFast) {
        const planned = planIGViewsFastBatches(quantity, remainingWindowMs, offsetMs);
        batchSizes = planned.batchSizes;
        scheduleTimes = planned.scheduleTimes;
        batchProviders = null;
      } else {
...
// Places an order with ChickletBoost. Matches their PHP client's `add`
// action: key, action=add, service, link, quantity. Returns their raw
// JSON response, e.g. { order: 12345 } on success or { error: "..." }.
async function placeChickletBoostOrder(env, { serviceId, link, quantity }) {
  return callChickletBoost(env, "add", {
    service: serviceId,
    link,
    quantity,
  });
...
function buildOrganicScheduleTimes(allSlots, count) {
...
// Orders whose window is under a full day don't fit the 24h fixed
// schedule well - it's built around ~1.5-2 hour gaps between anchors,
// so a 6-hour window might only contain 3-4 real anchor times. Forcing
// narrow-band (near-minimum) batch sizes into that few anchors is what
// caused batches to cluster within minutes of each other. Below this
// threshold, orders use planShortWindowBatches / the dual-provider
// short-window variant instead of the fixed SCHEDULE_BANK.
const SHORT_WINDOW_THRESHOLD_MS = 24 * 3600 * 1000;

// Distributes quantity across batchCount batches with real variation
// around the average (not a flat split), while guaranteeing every
// batch stays at or above effectiveMin and the total sums exactly to
// quantity. Used for short windows, where batch count is small and
// sizes need to scale UP to still deliver the full quantity in time.
function randomizeVariableBatchSizes(quantity, batchCount, effectiveMin) {
  const sizes = [];
  let remaining = quantity;
...
// Plans batches for a short (<24h) order: one batch per hour, max -
// never two batches in the same clock hour - with each batch landing
// at a genuinely random minute within its hour (not tied to the fixed
// schedule's anchor times, which are too sparse for short windows).
// Batch count is capped by however many whole hours actually fit in
// the window, so a high-quantity short order gets fewer, BIGGER
// batches instead of forcing tiny minimum-sized ones into too few slots.
function planShortWindowBatches(quantity, effectiveMin, windowMs, offsetMs) {
  const startDate = new Date(Date.now() + offsetMs);
  const numHourBuckets = Math.max(1, Math.floor(windowMs / 3600000));

  const bandWidth = Math.max(10, Math.round(effectiveMin * 0.4));
  const narrowBandCount = randomizeNarrowBandSizes(quantity, effectiveMin, bandWidth).length;
  const batchCount = Math.max(1, Math.min(narrowBandCount, numHourBuckets));

  // Pick which hours actually get a batch (all of them if batchCount
  // equals numHourBuckets, otherwise a random subset) so short orders
  // with fewer batches than hours don't just fill the first N hours.
  const chosenHours = shuffledIndices(numHourBuckets).slice(0, batchCount).sort((a, b) => a - b);

  const scheduleTimes = chosenHours.map((hourIndex) => {
    const hourStart = startDate.getTime() + hourIndex * 3600000;
    const hourEnd = Math.min(hourStart + 3600000, startDate.getTime() + windowMs);
    const spanSeconds = Math.max(1, Math.floor((hourEnd - hourStart) / 1000));
    return new Date(hourStart + randomInt(0, spanSeconds) * 1000);
  });

  const batchSizes = randomizeVariableBatchSizes(quantity, batchCount, effectiveMin);

  return { batchSizes, scheduleTimes };
}

// Dual-provider version of the short-window planner: same one-batch-
// per-hour rule, but each hour's batch also rolls a provider and is
// sized above THAT provider's own minimum.
function planShortWindowDualProviderBatches(quantity, betalogsMin, thekclautMin, windowMs, offsetMs) {
  ...
  const safeMin = Math.max(betalogsMin, thekclautMin);
  const maxBatches = Math.max(1, Math.floor(quantity / safeMin));
  const batchCount = Math.max(1, Math.min(maxBatches, numHourBuckets));
...
//
// Orders under SHORT_WINDOW_THRESHOLD_MS use planShortWindowBatches
// instead - see the comment above that constant for why.
function planScheduleBatches(scheduleIndex, quantity, effectiveMin, windowMs, offsetMs) {
  if (windowMs < SHORT_WINDOW_THRESHOLD_MS) {
    return planShortWindowBatches(quantity, effectiveMin, windowMs, offsetMs);
  }
  ...
  const scheduleTimes = buildOrganicScheduleTimes(allSlots, batchSizes.length);

  return { batchSizes, scheduleTimes };
}

// Rolls the gap (in minutes) before the next Instagram Views batch.
// Mostly lands in the normal 18-35 min band, but sometimes rolls a
// short "burst" gap or a longer "lull" gap so the spacing looks
// organic instead of a flat random range.
function rollIGViewsGapMinutes() {
  const roll = Math.random();
  if (roll < IG_VIEWS_BURST_CHANCE) {
    return randomInt(IG_VIEWS_BURST_GAP_MIN_MINUTES, IG_VIEWS_BURST_GAP_MAX_MINUTES);
  }
  if (roll < IG_VIEWS_BURST_CHANCE + IG_VIEWS_LULL_CHANCE) {
    return randomInt(IG_VIEWS_LULL_GAP_MIN_MINUTES, IG_VIEWS_LULL_GAP_MAX_MINUTES);
  }
  return randomInt(IG_VIEWS_GAP_MIN_MINUTES, IG_VIEWS_GAP_MAX_MINUTES);
}

// Plans Betalogs Instagram View batches: sequential sends with
// irregular gaps (see rollIGViewsGapMinutes) instead of the fixed
// SCHEDULE_BANK anchors, each batch sized IG_VIEWS_BATCH_MIN-
// IG_VIEWS_BATCH_MAX. If the run of sends would spill past the order's
// deadline, every overflow batch's quantity folds into the last batch
// that still lands inside the window, rather than scheduling a send
// past the deadline or silently dropping quantity.
function planIGViewsFastBatches(quantity, windowMs, offsetMs) {
  const startDate = new Date(Date.now() + offsetMs);
  const endDate = new Date(startDate.getTime() + windowMs);

  const bandWidth = IG_VIEWS_BATCH_MAX - IG_VIEWS_BATCH_MIN;
  const batchSizes = randomizeNarrowBandSizes(quantity, IG_VIEWS_BATCH_MIN, bandWidth);

  const scheduleTimes = [];
  let cursor = startDate;
  for (let i = 0; i < batchSizes.length; i++) {
    if (i > 0) {
      const gapMs = rollIGViewsGapMinutes() * 60000;
      cursor = new Date(cursor.getTime() + gapMs);
    }
    scheduleTimes.push(cursor);
  }

  const inWindow = scheduleTimes.filter((t) => t <= endDate).length;
  if (inWindow > 0 && inWindow < scheduleTimes.length) {
    const overflowQty = batchSizes.slice(inWindow).reduce((a, b) => a + b, 0);
    batchSizes.length = inWindow;
    scheduleTimes.length = inWindow;
    batchSizes[inWindow - 1] += overflowQty;
  }
...
//
// Orders under SHORT_WINDOW_THRESHOLD_MS use
// planShortWindowDualProviderBatches instead.
function planDualProviderScheduleBatches(scheduleIndex, quantity, betalogsMin, thekclautMin, windowMs, offsetMs) {
  if (windowMs < SHORT_WINDOW_THRESHOLD_MS) {
    return planShortWindowDualProviderBatches(quantity, betalogsMin, thekclautMin, windowMs, offsetMs);
  }
...
async function processDueBatches(env, force = false) {
  ...
  for (const batch of dueBatches) {
    try {
      ...
      } else if (batch.fields.provider === "chickletboost") {
        data = await placeChickletBoostOrder(env, {
          serviceId: batch.fields.serviceId,
          link: batch.fields.link,
          quantity: batch.fields.quantity,
        });
        if (data.error) throw new Error(data.error);
      } else {
