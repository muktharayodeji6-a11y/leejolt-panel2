...
function generateBatchPlan(totalQuantity, serviceMin) {
  ...
  // If the whole order is smaller than the service's own minimum,
  // it has to go out as a single batch - can't be split further.
  if (remaining <= effectiveMin) {
    return [remaining];
  }

  while (remaining > 0) {
    if (remaining <= BATCH_MAX || remaining <= effectiveMin) {
      batches.push(remaining);
      break;
    }
    const upperBound = Math.max(effectiveMin, BATCH_MAX);
    let size = randomInt(effectiveMin, upperBound);
    if (remaining - size < effectiveMin) size = remaining;
    batches.push(size);
    remaining -= size;
  }
  return batches;
