...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // TheKclaut Instagram Views: own fast drip pattern
      const isIGViewsThekclautFast = !dualProviderIds
        && targetProvider === "thekclaut"
        && String(serviceId) === IG_VIEWS_THEKCLAUT_SERVICE_ID;
      ...
      if (dualProviderIds) {
        ...
        const planned = planIGViewsFastBatches(quantity, offsetMs);
        batchSizes = planned.batchSizes;
        scheduleTimes = planned.scheduleTimes;
        batchProviders = null;
      } else if (isIGViewsThekclautFast) {
        const planned = planTheKclautInstagramViewsBatches(quantity, offsetMs);
        batchSizes = planned.batchSizes;
        scheduleTimes = planned.scheduleTimes;
        batchProviders = null;
      } else {
...
// Plans Betalogs Instagram View batches: sequential sends spaced
// IG_VIEWS_GAP_MIN_MINUTES-IG_VIEWS_GAP_MAX_MINUTES apart (ignores the
// fixed SCHEDULE_BANK anchors entirely), each batch sized
// IG_VIEWS_BATCH_MIN-IG_VIEWS_BATCH_MAX. Runs until every batch is
// placed - not capped to the order's deadline window, so a large
// quantity just spreads out for as long as it takes instead of getting
// squeezed or folded into a last oversized batch.
function planIGViewsFastBatches(quantity, offsetMs) {
  const startDate = new Date(Date.now() + offsetMs);
  ...
  for (let i = 0; i < batchSizes.length; i++) {
    if (i > 0) {
      const gapMs = randomInt(IG_VIEWS_GAP_MIN_MINUTES, IG_VIEWS_GAP_MAX_MINUTES) * 60000;
...
// Plans TheKclaut Instagram View batches: sequential sends spaced
// IG_VIEWS_THEKCLAUT_GAP_MIN_MINUTES-IG_VIEWS_THEKCLAUT_GAP_MAX_MINUTES apart,
// each batch sized IG_VIEWS_THEKCLAUT_BATCH_MIN-IG_VIEWS_THEKCLAUT_BATCH_MAX.
// Same organic spread pattern as Betalogs, but on TheKclaut's own timing config.
function planTheKclautInstagramViewsBatches(quantity, offsetMs) {
  const startDate = new Date(Date.now() + offsetMs);

  const bandWidth = IG_VIEWS_THEKCLAUT_BATCH_MAX - IG_VIEWS_THEKCLAUT_BATCH_MIN;
  const batchSizes = randomizeNarrowBandSizes(quantity, IG_VIEWS_THEKCLAUT_BATCH_MIN, bandWidth);

  const scheduleTimes = [];
  let cursor = startDate;
  for (let i = 0; i < batchSizes.length; i++) {
    if (i > 0) {
      const gapMs = randomInt(IG_VIEWS_THEKCLAUT_GAP_MIN_MINUTES, IG_VIEWS_THEKCLAUT_GAP_MAX_MINUTES) * 60000;
      cursor = new Date(cursor.getTime() + gapMs);
    }
    scheduleTimes.push(cursor);
  }
