...
// Distributes quantity across batchCount batches with randomized sizes
// (not an even split) while guaranteeing every batch stays at or above
// effectiveMin and the total sums exactly to quantity.
function randomizeBatchSizes(quantity, batchCount, effectiveMin) {
  const sizes = [];
  ...
  for (let i = 0; i < batchCount; i++) {
    ...
    // Never let this batch eat into what the remaining batches need to
...
// Maps an order's quantity onto its assigned fixed schedule: picks an
// evenly-spaced subset of the available slots (so delivery still spans
// the full window rather than front-loading) and distributes quantity
// across them with randomized batch sizes, respecting the service's
// real minimum per batch.
function planScheduleBatches(scheduleIndex, quantity, effectiveMin, windowMs, offsetMs) {
  ...
  const maxBatches = Math.max(1, Math.floor(quantity / effectiveMin));
  // Leave headroom below the theoretical max - hitting it exactly leaves
  // zero slack for randomizeBatchSizes to work with, so every batch
  // collapses to precisely the minimum instead of varying naturally.
  const safeMaxBatches = Math.max(1, Math.floor(maxBatches * 0.8));
  const batchCount = Math.min(safeMaxBatches, allSlots.length);

  const step = allSlots.length / batchCount;
  const scheduleTimes = Array.from({ length: batchCount }, (_, i) =>
    allSlots[Math.min(allSlots.length - 1, Math.floor(i * step))]
  );

  const batchSizes = randomizeBatchSizes(quantity, batchCount, effectiveMin);

  return { batchSizes, scheduleTimes };
}

// Same distribution logic as randomizeBatchSizes, but each batch can have
// its own minimum floor (mins[i]) rather than one shared minimum - needed
// because a batch's floor depends on which provider it gets randomly
// routed to (Betalogs vs TheKclaut have different real minimums).
function randomizeBatchSizesVariableMin(quantity, mins) {
  const batchCount = mins.length;
  const sizes = [];
  let remaining = quantity;
...
// Dual-provider version of planScheduleBatches: each batch is randomly
// routed to Betalogs or TheKclaut, and sized to respect whichever
// provider's real minimum it lands on.
//
// batchCount is deliberately capped using the LARGER of the two minimums
// (not the average) - that guarantees the plan stays feasible even in
// the worst case where every single coin flip happens to land on the
// provider with the higher floor (TheKclaut, 100). This trades a
// slightly lower max batch count for a guarantee that no batch ever
// gets created below its provider's real minimum.
//
// If the order's total quantity can't even cover one TheKclaut-minimum
// batch, TheKclaut is excluded entirely for that order - nothing to
// randomly route there in the first place.
function planDualProviderScheduleBatches(scheduleIndex, quantity, betalogsMin, thekclautMin, windowMs, offsetMs) {
  ...
  if (allSlots.length === 0) {
    // Window too short to contain even one scheduled slot - single batch,
    // right at window start.
  ...
  // Same headroom logic as the single-provider path - without it, batch
  // sizes collapse flat to the minimum when quantity is close to a clean
  // multiple of safeMin.
  const safeMaxBatches = Math.max(1, Math.floor(maxBatches * 0.8));
  const batchCount = Math.min(safeMaxBatches, allSlots.length);
  ...
  const batchSizes = randomizeBatchSizesVariableMin(quantity, mins);
